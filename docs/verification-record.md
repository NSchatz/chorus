...
toolchain                 rustc 1.98.0, cc (GCC) 12.2.0 for the endpoint's C
...
| AC-19, the suite passes on a machine with no radio, no ESP32-S3, no capture device and no privilege | `make check && make firmware-check` | Both green. `make check` builds the whole workspace and runs `cargo test --workspace`: 73 test binaries, 0 failed, including `wireless_policy` (9), `wireless_zones` (10), `report_shape` (16) and the impl gate's own `regress_0051_F1` (2). `make firmware-check` runs the endpoint's host build and every verification of it that needs no device: protocol 46 checks, sync core 137, safety scans 42, clocking 72, amplifier 86, telemetry 23, and **wireless bring-up 104**, all 0 failed, plus the real-socket outage run. The firmware half is named explicitly because `make check` is the cargo workspace only and would have gone green with every firmware criterion unbuilt |
...
| AC-9, an unbindable control address | `make verify` (`tools/refusals.sh`), and `cargo test -p chorus-server --test control_thread_population` | Against an address genuinely held by another process: exit **8**, which `crates/server/src/main.rs` documents, naming the address and `Address already in use (os error 98)`. It **served no audio**: there is no `listening on=` line at all, the audio address was never even reached - the check holds that port for the whole run, so a server that had got as far as its audio listener would have been refused on it and would have named it, and nothing in the output names it - and the run reports `chunks_sent=0`. It never said it was listening for control either, so it did not report itself as controllable. A state file this build cannot read stops it the same way, naming the file and the field that was missing |
