...
that matches nothing is COUNTED AND REPORTED as unexplained. There are none. The
classifier is shown able to say "unexplained" by
`the_classification_reports_a_resync_it_cannot_account_for` in the same file,
because a classification that can only say "explained" is not one.
...
make verify                   # every refusal path, including this phase's five
