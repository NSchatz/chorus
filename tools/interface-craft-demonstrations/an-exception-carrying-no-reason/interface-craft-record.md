# A record that excepts an entry and says nothing about why

base/interface-craft-record.md with one exception row added, carrying no reason.
The source beside it does carry Roboto, so the exception would be doing work if
it were granted. It is not granted: an exception without a stated reason is no
exception, and every place the entry is carried is reported.
