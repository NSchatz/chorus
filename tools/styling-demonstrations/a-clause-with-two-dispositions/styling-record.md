# A styling record with one clause claiming two things at once

A copy of the table in `docs/styling-conventions-record.md` with ONE thing
wrong: S7 carries an assertion AND an exemption. Two dispositions is not more
evidence than one, it is a row a reader cannot act on, and it is the shape an
exemption takes when somebody adds one beside an assertion instead of replacing
it.
