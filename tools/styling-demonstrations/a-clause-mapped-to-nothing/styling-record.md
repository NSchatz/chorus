# A styling record with a clause mapped to nothing

A copy of the table in `docs/styling-conventions-record.md` with ONE thing
wrong: S5 names no assertion, no exemption and no review disposition. An empty
row is the way a clause is dropped without anybody deciding to drop it, which is
why it is refused rather than read as "nothing to say".
